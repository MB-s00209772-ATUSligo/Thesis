...
class PropellerGeometry:
    ...
    def create_blade_section(self, r: float, theta: float = 0) -> np.ndarray:
        ...
        # Create airfoil shape (NACA-like)
        t = np.linspace(0, 2*np.pi, n_points)
        
        # Basic elliptical shape with thickness distribution
        airfoil_x = chord/2 * np.cos(t)
        airfoil_y = thickness/2 * np.sin(t)
        
        # Add camber
        airfoil_y += camber * (1 - (2*airfoil_x/chord)**2)
        ...
        # The blade section lies in a plane perpendicular to the radius
        points = []
        for i in range(len(airfoil_x)):
            # Apply pitch rotation (twist along the blade)
            x_rot = airfoil_x[i] * np.cos(pitch_angle) - airfoil_y[i] * np.sin(pitch_angle)
            z_rot = airfoil_x[i] * np.sin(pitch_angle) + airfoil_y[i] * np.cos(pitch_angle)
    ...
    def create_blade(self) -> trimesh.Trimesh:
        """Create a single blade mesh extending from the hub"""
        # Generate blade sections from hub to tip
        n_sections = 20
        r_hub = self.params.hub_dia_top/2 * 1.05  # Start slightly outside hub
        r_tip = self.params.diameter/2
        radii = np.linspace(r_hub, r_tip, n_sections)
        ...
        if len(vertices) > 0:
            ...
            tip_center = np.mean(vertices[last_section_start:], axis=0)
            ...
            for j in range(n_points):
                j_next = (j + 1) % n_points
                faces.append([last_section_start + j, last_section_start + j_next, tip_idx])
        
        # Cap the blade root (connection to hub)
        if len(vertices) > 0:
            first_section_end = len(section)
            root_center = np.mean(vertices[:first_section_end], axis=0)
            vertices.append(root_center)
            root_idx = len(vertices) - 1
            
            for j in range(len(section)):
                j_next = (j + 1) % len(section)
                faces.append([j_next, j, root_idx])
    ...
    def create_propeller(self) -> trimesh.Trimesh:
        ...
        for i in range(self.params.n_blades):
            # Create blade at specific angular position
        ...
        # Clean up and ensure watertight mesh
