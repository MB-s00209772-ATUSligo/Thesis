...
Fixed version with absolute paths and proper error handling
"""
...
class PropellerGeometry:
    ...
    def naca_airfoil(self, x, thickness, camber, camber_pos=0.4):
        ...
        thickness: max thickness as fraction of chord
        camber: max camber as fraction of chord
        camber_pos: position of max camber (0 to 1)
        """
    ...
    def create_blade_section(self, r: float, theta: float = 0) -> np.ndarray:
        ...
        # Vary thickness distribution - thicker at root, thinner at tip
        ...
        # Add extra thickness near root for structural strength
        if r_norm < 0.2:
            thickness_ratio *= (1 + 0.3 * (0.2 - r_norm) / 0.2)
        ...
        airfoil_x = np.concatenate([xu, xl[::-1][1:]])  # Skip duplicate leading edge point
        airfoil_y = np.concatenate([yu, yl[::-1][1:]])
        
        # Calculate pitch angle at this radius
        pitch_angle = np.arctan(self.params.pitch_ratio * self.params.diameter / (2 * np.pi * r)) if r > 0 else 0
        
        # Add variable twist distribution (more twist at root, less at tip)
        twist_factor = 1.0 - 0.3 * r_norm  # Reduce twist towards tip
        ...
        for i in range(len(airfoil_x)):
            ...
            # Apply sweep if near the tip (for reduced noise)
            if r_norm > 0.7:
                sweep_angle = (r_norm - 0.7) * 0.3 * np.pi / 6  # Up to 30 degrees sweep
                x_rot += chord * 0.2 * np.sin(sweep_angle)
            
            x = r * np.cos(angle) - x_rot * np.sin(angle)
            y = r * np.sin(angle) + x_rot * np.cos(angle)
            z = z_rot + self.params.hub_height/2
            
            # Apply rake (tilt of the blade)
            if self.params.rake != 0:
                rake_angle = self.params.rake * np.pi / 180
                z += r_norm * np.tan(rake_angle) * self.params.diameter/10
                # Add slight forward lean
                x += r_norm * np.sin(rake_angle) * self.params.diameter/30
    ...
    def create_blade_with_fillet(self) -> trimesh.Trimesh:
        ...
        r_start = self.params.hub_dia_base/2 * 0.8  # Start inside hub
        ...
        radii_root = np.linspace(r_start, r_hub, 5)  # Inside hub
        radii_fillet = np.linspace(r_hub, r_fillet_end, 8)  # Fillet region
        radii_blade = np.linspace(r_fillet_end, r_tip, n_sections - 13)  # Main blade
        ...
        if len(vertices) > 0:
            ...
            tip_radius = np.linalg.norm(np.array(tip_vertices) - tip_center, axis=1).mean() * 0.5
            
            # Add ring of vertices for rounded tip
            for v in tip_vertices:
                scaled_v = tip_center + (v - tip_center) * .5
        ...
        # Don't cap the root - let it intersect with hub
    ...
    def create_hub(self) -> trimesh.Trimesh:
        ...
        n_segments = 48
        n_height = 10  # Vertical segments for smoother surface
...
class BEMTSolver:
    ...
    def burrill_cavitation_criterion(self, r_norm, thrust_loading, sigma):
        ...
        if r_norm > 0.01:  # Small threshold to avoid zero
            projected_area_ratio = self.params.n_blades * chord / (2 * np.pi * r_norm * self.params.diameter/2000)
        else:
            projected_area_ratio = 0.1  # Default value at hub
...
class PropellerOptimizationProblem(Problem):
    ...
    def __init__(self):
        ...
        # Bounds for design variables - adjusted for better feasibility
        xl = np.array([0.18]*10 + [0.03]*10 + [0.0]*10 + [0.8, 0, 0, 3])
        xu = np.array([0.35]*10 + [0.10]*10 + [0.03]*10 + [1.4, 20, 8, 5])
        ...
        self.operating_conditions = {
            'V_ship': 3.0,  # m/s (reduced ship speed for small propeller)
            'rpm': 1500,  # RPM (increased for small propeller)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(n_pop):
            try:
                ...
                # Constraints (relaxed for 180mm propeller)
                g1[i] = 20 - results['thrust']  # Minimum thrust of 20N (realistic for 180mm)
                g2[i] = 0.025 - np.min(params.thickness_distribution)  # Minimum thickness ratio
            ...
            if self.eval_count % 100 == 0:
                print(f"Evaluation {self.eval_count}: Cav={f1[i]:.3f}, Eff={-f2[i]:.3f}, Noise={f3[i]:.1f}dB, Thrust constraint={g1[i]:.1f}")
...
def visualize_propeller(params: PropellerParams, save_path: str = None):
    ...
    # Don't show plot if running on HPC without display
    try:
        plt.show()
    except:
        plt.close()
...
def main():
    ...
    print("\nConstraints:")
    print("- Minimum thrust: 20 N (scaled for 180mm diameter)")
    ...
    # Initialize optimized_params with default values
    optimized_params = PropellerParams()
    best_solution_found = False
    
    # Handle results - check if results exists and has required attributes
    if results is not None:
        try:
            if hasattr(results, 'X') and results.X is not None and len(results.X) > 0:
                if hasattr(results, 'F') and results.F is not None and len(results.F) > 0:
                    # We have valid results
                    best_solution_found = True
                    
                    # Check for feasible solutions
                    if hasattr(results, 'CV') and results.CV is not None:
                        # Check for feasible solutions (CV <= 0)
                        feasible_mask = np.all(results.CV <= 0, axis=1)
                        
                        if np.any(feasible_mask):
                            # Use feasible solutions
                            feasible_F = results.F[feasible_mask]
                            feasible_X = results.X[feasible_mask]
                            
                            # Weight objectives among feasible solutions
                            weights = np.array([0.4, 0.4, 0.2])
                            F_norm = feasible_F.copy()
                            for i in range(3):
                            ...
                            print("Using best feasible solution...")
                        else:
                            # No feasible solutions, use least infeasible
                            cv_sum = np.sum(np.abs(results.CV), axis=1)
                            best_idx = np.argmin(cv_sum)
                            best_solution = results.X[best_idx]
                            print("Warning: No fully feasible solutions found. Using least infeasible solution...")
                            print(f"Constraint violation: {cv_sum[best_idx]:.3f}")
                    else:
                        # No constraint violation info, use multi-objective ranking
                        weights = np.array([0.4, 0.4, 0.2])
                        ...
                        best_idx = np.argmin(scores)
                        best_solution = results.X[best_idx]
                        print("Using best multi-objective solution...")
                    
                    # Create optimized parameters from best solution
                    ...
                    print("Warning: Optimization returned no objective values.")
                    print("Using default parameters...")
            else:
                print("Warning: Optimization returned no solutions.")
                print("Using default parameters...")
        except Exception as e:
            print(f"Error processing optimization results: {e}")
            print("Using default parameters...")
    else:
        print("Warning: Optimization did not return results.")
        print("Using default parameters...")
    
    # Ensure reasonable default values if not optimized
    if not best_solution_found or optimized_params.chord_distribution is None:
        print("\nSetting default blade distributions...")
        r = np.linspace(0.2, 1.0, 10)
        optimized_params.chord_distribution = 0.25 - 0.15 * r + 0.08 * r**2
        optimized_params.thickness_distribution = 0.05 - 0.04 * r
        optimized_params.camber_distribution = 0.015 * np.ones_like(r)
        optimized_params.n_blades = 4
        optimized_params.pitch_ratio = 1.0
        optimized_params.skew = 10.0
        optimized_params.rake = 5.0
    
    # Calculate final performance with adjusted conditions
    solver = BEMTSolver(optimized_params, {
        'V_ship': 3.0,  # Adjusted for small propeller
        'rpm': 1500,
        'rho': 1025.0,
        'nu': 1.35e-6,
        'depth': 5.0,
    ...
    try:
        ...
        print("="*60)
        print(f"✓ {stl_path}")
        print(f"  └─ 3D model for manufacturing (STL format)")
        print(f"✓ {threedf_path}")
        print(f"  └─ Alternative 3D format (3MF)")
        print(f"✓ {json_path}")
        print(f"  └─ Design parameters & performance data (JSON)")
        if os.path.exists(png_path):
            print(f"✓ {png_path}")
            print(f"  └─ Visualization & performance metrics (PNG)")
        print(f"\nAll files saved in: {output_dir}")
        print("\nThe propeller has been optimized for minimal cavitation")
        print("and noise while maintaining efficiency for a 180mm marine propeller.")
        
        # List files in output directory to confirm
        print("\nFiles in output directory:")
        try:
            files = os.listdir(output_dir)
            for f in files:
                if 'optimized_propeller' in f:
        ...
        print("Please check write permissions and required libraries.")
